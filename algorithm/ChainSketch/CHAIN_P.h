...
#define KEY_SIZE 4
   ...
   public:
        ...
        void insert(uint32_t key) {
            ...
            if (randomVal <= 1.0 / (min_counter + 1)) {
                ...
                if (min_counter >= 512) {
                    ...
                    while (l <= MAX_CHAIN_P_LENGTH && tempSet.find(p2) == tempSet.end() && min_counter > bucketArray[min_rowIdx][p2].C) {
                        tempSet.insert(p2);
    ...
    CHAIN_P(int k, double total_memory, uint32_t* seeds, int data_id = 0) {
        srand((int)time(0));
    ...
    CHAIN_P(int n, int m, double total_memory, uint32_t* seeds, int data_id = 0) {
        srand((int)time(0));
    ...
    void send(uint32_t f, uint32_t e) {
        ...
        for (int i = 0; i < current_path.size(); i++) {
            ...
            if (flag == 0) {
                double hashV = H(f, 31);
                hashV /= (double)MAXI;
                if (!(i != current_path.size() - 1 && hashV >= prob[current_switch])) {
