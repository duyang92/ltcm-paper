#ifndef _CHAIN_OUR_P_
#define _CHAIN_OUR_P_
   ...
   public:
    ...
    class hg_node {
       public:
        ...
        uint32_t ROW_NUM;
        uint32_t COL_NUM;
        uint32_t MAX_CHAIN_OUR_P_LENGTH = 3;
        ...
        void insert(uint32_t key) {
            ...
            if (randomVal <= 1.0 / (min_counter + 1)) {
                ...
                if (min_counter >= 512) {
                    ...
                    while (l <= MAX_CHAIN_OUR_P_LENGTH && tempSet.find(p2) == tempSet.end() && min_counter > bucketArray[min_rowIdx][p2].C) {
                        tempSet.insert(p2);
    ...
    CHAIN_OUR_P(int k, double total_memory, uint32_t* seeds, uint32_t re_dis_th = 10000) {
        srand((int)time(0));
    ...
    CHAIN_OUR_P(int n, int m, double total_memory, uint32_t* seeds, uint32_t re_dis_th = 10000) {
        srand((int)time(0));
