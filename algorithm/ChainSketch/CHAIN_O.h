...
#define KEY_SIZE 4
   ...
   public:
        ...
        void insert(uint32_t key) {
            ...
            if (randomVal <= 1.0 / (min_counter + 1)) {
                ...
                if (min_counter >= 512) {
                    ...
                    while (l <= MAX_CHAIN_O_LENGTH && tempSet.find(p2) == tempSet.end() && min_counter > bucketArray[min_rowIdx][p2].C) {
                        tempSet.insert(p2);
    ...
    CHAIN_O(int k, double total_memory, uint32_t* seeds, int data_id = 0) {
        srand((int)time(0));
    ...
    CHAIN_O(int n, int m, double total_memory, uint32_t* seeds, int data_id = 0) {
        srand((int)time(0));
