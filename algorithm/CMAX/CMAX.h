...
#define MAXI 4294967295
   ...
   public:
    ...
    CMAX(int k, double memory, uint32_t* seeds) {
        srand((int)time(0));
    ...
    CMAX(int n, int m, double memory, uint32_t* seeds) {
        srand((int)time(0));
    ...
    void send(uint32_t f, uint32_t e) {
        ...
        for (int i = 0; i < current_path.size(); i++) {
            ...
            if (hashV % 2 == 0 && i != 0) continue;
            if (hashV % 2 == 1 && i != current_path.size() - 1) continue;
