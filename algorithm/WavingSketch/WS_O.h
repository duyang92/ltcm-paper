...
#define KEY_SIZE 4
   ...
   public:
    ...
    WS_O(int k, double total_memory, uint32_t* seeds, int data_id = 0) {
        srand((int)time(0));
    ...
    WS_O(int n, int m, double total_memory, uint32_t* seeds, int data_id = 0) {
        srand((int)time(0));
    ...
    uint32_t query(uint32_t f) {
        ...
        for (int i = 0; i < current_path.size(); i++) {
            int current_switch = current_path[i];
            res = max(res, swtich_hg[current_switch][hash % bucketNum].query(f));
        }
        if (res < 1) res = 1;
        return res;
    }
