...
#define KEY_SIZE 4
   ...
   public:
    ...
    WS_OUR(int k, double total_memory, uint32_t* seeds, int re_dis_th = 10000) {
        srand((int)time(0));
        ...
        this->threshold = this->reset * 0.001;
    ...
    WS_OUR(int n, int m, double total_memory, uint32_t* seeds, int re_dis_th = 10000) {
        srand((int)time(0));
    ...
    uint32_t query(uint32_t f) {
        ...
        vector<int> current_path = path[path_id];
        uint32_t hash_value = H(f, index_seed);
        uint32_t s1 = 0, s2 = 1e9;
        for (int i = 0; i < current_path.size(); i++) {
            uint32_t hash = hash_value;
            uint16_t fp = FP(hash);
            if (interval[path_id][i].first <= hash_value && hash_value <= interval[path_id][i].second) {
                s1 = swtich_hg[current_path[i]][hash % bucketNum].query(f);
            }
        }
        return s1;
    }
